...
class Settings:
	...
	def set_settings(cls, view: sublime.View, variables: Dict[str, str]) -> None:
		...
		variables.update({k: v for k, v in cls.data["config"].items() if "." not in k and type(v) == str})
	...
	def verify_settings(cls) -> None:
		...
		for k, v in cls.data["config"].items():
			if type(v) == str and "node_" in k and "path" in k and PLATFORM.lower() in k:
				node_path_exist = True if os_path.exists(v) else (k, v)

			if type(v) == str and "stylelint_" in k and "path" in k and PLATFORM.lower() in k:
				if "local" in k:
...
class FormatStylelintCommand(sublime_plugin.TextCommand):
	def run(self, edit) -> None:
		...
		cmd_extra = list(
			...
			["--report-descriptionless-disables", "--report-invalid-scope-disables", "--report-needless-disables", "--fix"]
